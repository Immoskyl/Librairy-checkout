	...
	public ArrayList<newUser> readStaff(String Filename)
	{
		...
		try{
	      ...
	      while(br.ready())
	      {
	        fileLines = br.readLine();
	        parts = fileLines.split(",");
	        Staff aStaff = new Staff(parts[0], parts[1], parts[2], parts[3], parts[4]);
	        staff.add(aStaff);
	      }
	      br.close();
	      } 
		...
		try{
	      ...
	      while(br.ready())
	      {
	    	//calling factory for admin
	    	  newUserFactory userFactory = new newUserFactory();
	    	  newUser aUser = null;
	        fileLines = br.readLine();
	        parts = fileLines.split(",");
	        newUser aAdministrator = newUserFactory.makeNewUser("a", parts[0], parts[1], parts[2], parts[3], parts[4]);
