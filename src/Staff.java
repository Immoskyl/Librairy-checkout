...
public class Staff extends newUser{

	Staff(){
		super();
	}

	Staff(String name ,String address , String email , String userId, String password){
		setName(name);
		setAddress(address);
		setEmail(email);
		setUserID(userId);
	...
	public Staff createStaff(String name, String add, String e, String ID, String p) {
		Staff s = new Staff(name.trim() , getAddress().trim() , getEmail().trim() , getUserId().trim(), getPassword().trim());
		return s;
	}
